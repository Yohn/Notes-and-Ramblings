---
created: 2024-09-03T23:51:40-04:00
modified: 2024-09-04T00:11:36-04:00
---

```js RunJS="{n:'Mods/Callouts',t:'m'}"
...
export async function insertCallout(app, run_type = 2) {
  ...
  class ListModal extends SuggestModal {
    ...
    getSuggestions(query) {
      return CalloutTypes.filter((type) => type[0].toLowerCase().includes(query.toLowerCase()))
    }
  ...
  // Run Type: 1 ##################################################
    ...
    const runJS = app.plugins.plugins["yohns-web"]
      ...
      onOpen() {
        ...
        this.fold_radios.push(
          div_head.createEl("input", {
            attr: { type: "radio", id: "NoFold", name: "foldState", value: "0" },
          }),
        )
        div_head.createEl("label", { text: "No", attr: { for: "NoFold" } })
        this.fold_radios.push(
          div_head.createEl("input", {
            attr: { type: "radio", id: "Expanded", name: "foldState", value: "1" },
          }),
        )
        div_head.createEl("label", { text: "➕", attr: { for: "Expanded" } })
        this.fold_radios.push(
          div_head.createEl("input", {
            attr: { type: "radio", id: "Collapsed", name: "foldState", value: "2" },
          }),
        )
        ...
        this.title = callout_title.createDiv({
          ...
          attr: { contenteditable: "plaintext-only", placeholder: "Enter title here" },
        })
        ...
        this.callout_editor = callout_content.createEl("p", {
          attr: { contenteditable: "plaintext-only", placeholder: "Enter content here" },
        })
  ...
  // Run Type: 3 ##################################################
    ...
    if (templater) {
      ...
      foldState = await tp.system.suggester(
        ["Disable folding", "Enable folding (expanded state)", "Enable folding (collapsed state)"],
        ["", "+", "-"],
        false,
        "Would you like to use the callout collapse feature?",
      )
      ...
      title = await tp.system.prompt("What is the callout title? (optional)", "")
      if (title == null) {
        title = ""
      }

      calloutContent = await tp.system.prompt(
        "What is the callout content? (Optional, Shift + Enter for line break)",
        "",
        false,
        true,
      )
