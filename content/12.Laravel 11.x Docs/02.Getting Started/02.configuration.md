---
modified: 2024-09-06T05:32:32-04:00
tags:
  - laravel
  - laravel11
  - docs
...
However, it is possible to encrypt your environment file using Laravel's built-in [environment encryption](<#environment encryption>). Encrypted environment files may be placed in source control safely.
...
The current application environment is determined via the `APP_ENV` variable from your `.env` file. You may access this value via the `environment` method on the `App` [facade](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/05.facades):
...
> [!NOTE]
> The length of the key provided should match the key length required by the encryption cipher being used. By default, Laravel will use the `AES-256-CBC` cipher which requires a 32 character key. You are free to use any cipher supported by Laravel's [encrypter](/12.Laravel%2011.x%20Docs/06.Security/04.) by passing the `--cipher` option when invoking the command.
...
While your application is in maintenance mode, no [queued jobs](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/16.queues) will be handled. The jobs will continue to be handled as normal once the application is out of maintenance mode.
