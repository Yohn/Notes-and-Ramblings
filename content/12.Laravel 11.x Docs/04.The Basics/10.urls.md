---
modified: 2024-09-14T02:03:38-04:00
tags:
  - laravel
  - laravel11
  - docs
...
Each of these methods may also be accessed via the `URL` [facade](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/05.facades):
...
The `route` helper may be used to generate URLs to [named routes](/12.Laravel%2011.x%20Docs/04.The%20Basics/01.routing#named-routes). Named routes allow you to generate URLs without being coupled to the actual URL defined on the route. Therefore, if the route's URL changes, no changes need to be made to your calls to the `route` function. For example, imagine your application contains a route defined like the following:
...
You will often be generating URLs using the route key (typically the primary key) of [Eloquent models](/12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/01.eloquent). For this reason, you may pass Eloquent models as parameter values. The `route` helper will automatically extract the model's route key:
