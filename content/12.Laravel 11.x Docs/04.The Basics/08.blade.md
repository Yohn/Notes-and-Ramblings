---
modified: 2024-09-14T04:27:17-04:00
tags:
  - laravel
  - laravel11
  - docs
...
- [Anonymous Components](<#Anonymous Components>)
  ...
  - [Anonymous Components Paths](<#Anonymous Components Paths>)
...
Blade views may be returned from routes or controllers using the global `view` helper. Of course, as mentioned in the documentation on [views](/12.Laravel%2011.x%20Docs/04.The%20Basics/07.views), data may be passed to the Blade view using the `view` helper's second argument:
...
By default, Blade `blade {{ }}` statements are automatically sent through PHP's `htmlspecialchars` function to prevent XSS attacks. If you do not want your data to be escaped, you may use the following syntax:
...
The `@auth` and `@guest` directives may be used to quickly determine if the current user is [authenticated](/12.Laravel%2011.x%20Docs/06.Security/01.authentication) or is a guest:
...
The `@session` directive may be used to determine if a [session](/12.Laravel%2011.x%20Docs/04.The%20Basics/11.session) value exists. If the session value exists, the template contents within the `@session` and `@endsession` directives will be evaluated. Within the `@session` directive's contents, you may echo the `$value` variable to display the session value:
...
> [!NOTE]
> While iterating through a `foreach` loop, you may use the [loop variable](<#loop variable>) to gain valuable information about the loop, such as whether you are in the first or last iteration through the loop.
...
If your component requires dependencies from Laravel's [service container](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/02.container), you may list them before any of the component's data attributes and they will automatically be injected by the container:
...
> [!NOTE]
> If you need to conditionally compile classes on other HTML elements that shouldn't receive merged attributes, you can use the [`@class` directive](<#`@class` directive>).
...
Like Blade components, you may assign additional [attributes](#attributes) to slots such as CSS class names:
...
Since anonymous components do not have any associated class, you may wonder how you may differentiate which data should be passed to the component as variables and which attributes should be placed in the component's [attribute bag](<#attribute bag>).
...
The `anonymousComponentPath` method accepts the "path" to the anonymous component location as its first argument and an optional "namespace" that components should be placed under as its second argument. Typically, this method should be called from the `boot` method of one of your application's [service providers](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/03.providers):
...
Anytime you define an HTML form in your application, you should include a hidden CSRF token field in the form so that [the CSRF protection](/12.Laravel%2011.x%20Docs/04.The%20Basics/03.csrf) middleware can validate the request. You may use the `@csrf` Blade directive to generate the token field:
...
The `@error` directive may be used to quickly check if [validation error messages](/12.Laravel%2011.x%20Docs/04.The%20Basics/12.validation#quick-displaying-the-validation-errors) exist for a given attribute. Within an `@error` directive, you may echo the `$message` variable to display the error message:
...
You may pass [the name of a specific error bag](</12.Laravel%2011.x%20Docs/04.The%20Basics/12.validation#named error bags>) as the second parameter to the `@error` directive to retrieve validation error messages on pages containing multiple forms:
...
The `@inject` directive may be used to retrieve a service from the Laravel [service container](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/02.container). The first argument passed to `@inject` is the name of the variable the service will be placed into, while the second argument is the class or interface name of the service you wish to resolve:
