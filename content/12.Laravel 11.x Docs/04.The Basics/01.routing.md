---
modified: 2024-09-07T01:38:13-04:00
tags:
  - laravel
  - laravel11
  - docs
...
All Laravel routes are defined in your route files, which are located in the `routes` directory. These files are automatically loaded by Laravel using the configuration specified in your application's `bootstrap/app.php` file. The `routes/web.php` file defines routes that are for your web interface. These routes are assigned the `web` [middleware group](</12.Laravel%2011.x%20Docs/04.The%20Basics/02.middleware#Laravel's Default Middleware Groups>), which provides features like session state and CSRF protection.
...
You may type-hint any dependencies required by your route in your route's callback signature. The declared dependencies will automatically be resolved and injected into the callback by the Laravel [service container](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/02.container). For example, you may type-hint the `Illuminate\Http\Request` class to have the current HTTP request automatically injected into your route callback:
...
Remember, any HTML forms pointing to `POST`, `PUT`, `PATCH`, or `DELETE` routes that are defined in the `web` routes file should include a CSRF token field. Otherwise, the request will be rejected. You can read more about CSRF protection in the [CSRF documentation](/12.Laravel%2011.x%20Docs/04.The%20Basics/03.csrf):
...
> [!NOTE]
> Sometimes, you may wish to specify request-wide default values for URL parameters, such as the current locale. To accomplish this, you may use the [`URL::defaults` method](/12.Laravel%2011.x%20Docs/04.The%20Basics/10.urls#default-values).
...
To assign [middleware](/12.Laravel%2011.x%20Docs/04.The%20Basics/02.middleware) to all routes within a group, you may use the `middleware` method before defining the group. Middleware are executed in the order they are listed in the array:
...
If a group of routes all utilize the same [controller](/12.Laravel%2011.x%20Docs/04.The%20Basics/04.controllers.md), you may use the `controller` method to define the common controller for all of the routes within the group. Then, when defining the routes, you only need to provide the controller method that they invoke:
...
Typically, implicit model binding will not retrieve models that have been [soft deleted](</12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/01.eloquent#soft deleting>). However, you may instruct the implicit binding to retrieve these models by chaining the `withTrashed` method onto your route's definition:
...
If a route is utilizing [implicit binding scoping](<#Custom Keys and Scoping>), the `resolveChildRouteBinding` method will be used to resolve the child binding of the parent model:
...
Rate limiters may be attached to routes or route groups using the `throttle` [middleware](/12.Laravel%2011.x%20Docs/04.The%20Basics/02.middleware). The throttle middleware accepts the name of the rate limiter you wish to assign to the route:
...
For convenience, you may use the `@method` [Blade directive](/12.Laravel%2011.x%20Docs/04.The%20Basics/08.blade) to generate the `_method` input field:
...
Laravel can automatically respond to CORS `OPTIONS` HTTP requests with values that you configure. The `OPTIONS` requests will automatically be handled by the `HandleCors` [middleware](/12.Laravel%2011.x%20Docs/04.The%20Basics/02.middleware) that is automatically included in your application's global middleware stack.
