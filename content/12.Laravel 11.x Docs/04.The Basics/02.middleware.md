---
modified: 2024-09-06T23:24:27-04:00
tags:
  - laravel
  - laravel11
  - docs
...
> [!NOTE]
> All middleware are resolved via the [service container](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/02.container), so you may type-hint any dependencies you need within a middleware's constructor.
...
You may also exclude a given set of middleware from an entire [group](/12.Laravel%2011.x%20Docs/04.The%20Basics/01.routing#route-groups) of route definitions:
...
When calling the `terminate` method on your middleware, Laravel will resolve a fresh instance of the middleware from the [service container](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/02.container). If you would like to use the same middleware instance when the `handle` and `terminate` methods are called, register the middleware with the container using the container's `singleton` method. Typically this should be done in the `register` method of your `AppServiceProvider`:
