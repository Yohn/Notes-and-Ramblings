---
modified: 2024-09-06T05:44:55-04:00
tags:
  - laravel
  - laravel11
  - docs
...
- [Live Validation](<#Live Validation>)
  ...
  - [Using Alpine and Blade](<#Using Alpine and Blade>)
...
Laravel Precognition allows you to anticipate the outcome of a future HTTP request. One of the primary use cases of Precognition is the ability to provide "live" validation for your frontend JavaScript application without having to duplicate your application's backend validation rules. Precognition pairs especially well with Laravel's Inertia-based [starter kits](/12.Laravel%2011.x%20Docs/02.Getting%20Started/05.starter-kits).

When Laravel receives a "precognitive request", it will execute all of the route's middleware and resolve the route's controller dependencies, including validating [form requests](/12.Laravel%2011.x%20Docs/04.The%20Basics/12.validation#form-request-validation) - but it will not actually execute the route's controller method.
...
First, to enable Precognition for a route, the `HandlePrecognitiveRequests` middleware should be added to the route definition. You should also create a [form request](/12.Laravel%2011.x%20Docs/04.The%20Basics/12.validation#form-request-validation) to house the route's validation rules:
...
> [!NOTE]
> If you would like a head start when developing your Laravel application with Vue and Inertia, consider using one of our [starter kits](/12.Laravel%2011.x%20Docs/02.Getting%20Started/05.starter-kits). Laravel's starter kits provide backend and frontend authentication scaffolding for your new Laravel application.

Before using Precognition with Vue and Inertia, be sure to review our general documentation on [using Precognition with Vue](<#using Precognition with Vue>). When using Vue with Inertia, you will need to install the Inertia compatible Precognition library via NPM:
...
Before using Precognition with React and Inertia, be sure to review our general documentation on [using Precognition with React](<#using Precognition with React>). When using React with Inertia, you will need to install the Inertia compatible Precognition library via NPM:
...
Because of this behavior, you should ensure that your application [customizes the corresponding form request's validation rules](<#customizes the corresponding form request's validation rules>) to specify the field is only required for full form submissions:
