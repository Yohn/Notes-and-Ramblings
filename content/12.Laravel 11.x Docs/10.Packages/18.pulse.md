---
modified: 2024-09-06T05:44:26-04:00
tags:
  - laravel
  - laravel11
  - docs
...
- [Performance](#Performance)
  ...
  - [Sampling](#Sampling)
  - [Trimming](#Trimming)
  - [Handling Pulse Exceptions](<#Handling Pulse Exceptions>)
...
For in-depth debugging of individual events, check out [Laravel Telescope](/12.Laravel%2011.x%20Docs/10.Packages/23.telescope).
...
> [!NOTE]
> If you do not want to store Pulse data in your application's primary database, you may [specify a dedicated database connection](<#specify a dedicated database connection>).
...
> [!NOTE]
> You may completely customize how the authenticated user is captured and retrieved by implementing the `Laravel\Pulse\Contracts\ResolvesUsers` contract and binding it in Laravel's [service container](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/02.container#binding-a-singleton).
...
To learn how to customize how Pulse retrieves and displays user information, consult our documentation on [resolving users](<#resolving users>).
...
The `<livewire:pulse.slow-outgoing-requests />` card shows outgoing requests made using Laravel's [HTTP client](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/10.http-client) that exceed the configured threshold, which is 1,000ms by default.
...
> [!NOTE]
> Pulse uses the [cache](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/03.cache) to store restart signals, so you should verify that a cache driver is properly configured for your application before using this feature.

<a name="recorders"></a>

### Recorders

Recorders are responsible for capturing entries from your application to be recorded in the Pulse database. Recorders are registered and configured in the `recorders` section of the [Pulse configuration file](<#Pulse configuration file>).

<a name="cache-interactions-recorder"></a>

#### Cache Interactions

The `CacheInteractions` recorder captures information about the [cache](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/03.cache) hits and misses occurring in your application for display on the [Cache](<#cache](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/03.cache) hits and misses occurring in your application for display on the [Cache>) card.

You may optionally adjust the [sample rate](<#sample rate>) and ignored key patterns.
...
The `Exceptions` recorder captures information about reportable exceptions occurring in your application for display on the [Exceptions](#Exceptions) card.

You may optionally adjust the [sample rate](<#sample rate>) and ignored exceptions patterns. You may also configure whether to capture the location that the exception originated from. The captured location will be displayed on the Pulse dashboard which can help to track down the exception origin; however, if the same exception occurs in multiple locations then it will appear multiple times for each unique location.

<a name="queues-recorder"></a>

#### Queues

The `Queues` recorder captures information about your applications queues for display on the [Queues](#Queues).

You may optionally adjust the [sample rate](<#sample rate>) and ignored jobs patterns.

<a name="slow-jobs-recorder"></a>

#### Slow Jobs

The `SlowJobs` recorder captures information about slow jobs occurring in your application for display on the [Slow Jobs](<#Slow Jobs>) card.

You may optionally adjust the slow job threshold, [sample rate](<#sample rate>), and ignored job patterns.
...
The `SlowOutgoingRequests` recorder captures information about outgoing HTTP requests made using Laravel's [HTTP client](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/10.http-client) that exceed the configured threshold for display on the [Slow Outgoing Requests](<#HTTP client](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/10.http-client) that exceed the configured threshold for display on the [Slow Outgoing Requests>) card.

You may optionally adjust the slow outgoing request threshold, [sample rate](<#sample rate>), and ignored URL patterns.
...
The `Requests` recorder captures information about requests made to your application for display on the [Slow Requests](<#Slow Requests>) and [Application Usage](<#Application Usage>) cards.

You may optionally adjust the slow route threshold, [sample rate](<#sample rate>), and ignored paths.
...
The `Servers` recorder captures CPU, memory, and storage usage of the servers that power your application for display on the [Servers](#Servers) card. This recorder requires the [`pulse:check` command](<#`pulse:check` command>) to be running on each of the servers you wish to monitor.
...
The `UserJobs` recorder captures information about the users dispatching jobs in your application for display on the [Application Usage](<#Application Usage>) card.

You may optionally adjust the [sample rate](<#sample rate>) and ignored job patterns.

<a name="user-requests-recorder"></a>

#### User Requests

The `UserRequests` recorder captures information about the users making requests to your application for display on the [Application Usage](<#Application Usage>) card.
...
You may customize the [database connection](/12.Laravel%2011.x%20Docs/07.Database/01.database#configuration) used by Pulse by setting the `PULSE_DB_CONNECTION` environment variable.
...
By default, Pulse will store entries directly to the [configured database connection](<#configured database connection>) after the HTTP response has been sent to the client or a job has been processed; however, you may use Pulse's Redis ingest driver to send entries to a Redis stream instead. This can be enabled by configuring the `PULSE_INGEST_DRIVER` environment variable:

```
PULSE_INGEST_DRIVER=redis
```

Pulse will use your default [Redis connection](/12.Laravel%2011.x%20Docs/07.Database/06.redis#configuration) by default, but you may customize this via the `PULSE_REDIS_CONNECTION` environment variable:
...
You may instead choose to enable "sampling" on certain Pulse data recorders. For example, setting the sample rate to `0.1` on the [`User Requests`](<#`User Requests`>) recorder will mean that you only record approximately 10% of the requests to your application. In the dashboard, the values will be scaled up and prefixed with a `~` to indicate that they are an approximation.
...
Pulse will automatically trim its stored entries once they are outside of the dashboard window. Trimming occurs when ingesting data using a lottery system which may be customized in the Pulse [configuration file](<#configuration file>).
...
Once you have defined your Livewire component and template, the card may be included in your [dashboard view](<#dashboard view>):
...
If your custom card lives within your application's code base and you are using Laravel's [Vite integration](/12.Laravel%2011.x%20Docs/04.The%20Basics/09.vite), you may update your `vite.config.js` file to include a dedicated CSS entry point for your card:
...
You may then use the `@vite` Blade directive in your [dashboard view](<#dashboard view>), specifying the CSS entrypoint for your card:
...
> [!NOTE]
> When building a card package that captures the currently authenticated user ID, you should use the `Pulse::resolveAuthenticatedUserId()` method, which respects any [user resolver customizations](<#user resolver customizations>) made to the application.
