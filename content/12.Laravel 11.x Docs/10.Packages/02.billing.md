---
modified: 2024-09-06T06:54:05-04:00
tags:
  - laravel
  - laravel11
  - docs
...
Lastly, to ensure Cashier properly handles all Stripe events, remember to [configure Cashier's webhook handling](<#configure Cashier's webhook handling>).
...
> [!WARNING]
> If you're using a model other than Laravel's supplied `App\Models\User` model, you'll need to publish and alter the [Cashier migrations](<#Cashier migrations>) provided to match your alternative model's table name.
...
> [!NOTE]
> Before utilizing Stripe Checkout, you should define Products with fixed prices in your Stripe dashboard. In addition, you should [configure Cashier's webhook handling](<#configure Cashier's webhook handling>).
...
After the card has been verified by Stripe, you may pass the resulting `setupIntent.payment_method` identifier to your Laravel application, where it can be attached to the customer. The payment method can either be [added as a new payment method](<#added as a new payment method>) or [used to update the default payment method](<#used to update the default payment method>). You can also immediately use the payment method identifier to [create a new subscription](<#create a new subscription>).
...
If the card is verified successfully, you may pass the `paymentMethod.id` to your Laravel application and process a [single charge](<#single charge>).
...
> [!NOTE]
> To learn how to retrieve payment method identifiers please review the [payment method storage documentation](<#payment method storage documentation>).
...
The `create` method, which accepts [a Stripe payment method identifier](<#a Stripe payment method identifier>) or Stripe `PaymentMethod` object, will begin the subscription as well as update your database with the billable model's Stripe customer ID and other relevant billing information.
...
If you would like to update quantities on individual subscription prices, you may do so using the [existing quantity methods](<#existing quantity methods>) by passing the ID of the price as an additional argument to the method:
...
You may also start a metered subscription via [Stripe Checkout](<#Stripe Checkout>):
...
> [!WARNING]
> Instead of calculating Tax Rates manually, you can [automatically calculate taxes using Stripe Tax](<#automatically calculate taxes using Stripe Tax>)
...
This will also sync any item tax rates for a subscription with multiple products. If your application is offering subscriptions with multiple products, you should ensure that your billable model implements the `priceTaxRates` method [discussed above](<#discussed above>).
...
> [!WARNING]
> Be sure to add a [date cast](/12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/04.eloquent-mutators#date-casting) for the `trial_ends_at` attribute within your billable model's class definition.
...
> [!WARNING]
> Make sure you protect incoming Stripe webhook requests with Cashier's included [webhook signature verification](<#webhook signature verification>) middleware.

<a name="webhooks-csrf-protection"></a>

#### Webhooks and CSRF Protection

Since Stripe webhooks need to bypass Laravel's [CSRF protection](/12.Laravel%2011.x%20Docs/04.The%20Basics/03.csrf), you should ensure that Laravel does not attempt to validate the CSRF token for incoming Stripe webhooks. To accomplish this, you should exclude `stripe/*` from CSRF protection in your application's `bootstrap/app.php` file:
...
Both events contain the full payload of the Stripe webhook. For example, if you wish to handle the `invoice.payment_succeeded` webhook, you may register a [listener](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/07.events#defining-listeners) that will handle the event:
...
If you would like to make a one-time charge against a customer, you may use the `charge` method on a billable model instance. You will need to [provide a payment method identifier](<#provide a payment method identifier>) as the second argument to the `charge` method:
...
If you need to refund a Stripe charge, you may use the `refund` method. This method accepts the Stripe [payment intent ID](<#payment intent ID>) as its first argument:
...
> [!WARNING]
> If you have already configured [automatic tax collection](<#automatic tax collection>) in your application's service provider then this feature will be enabled automatically and there is no need to invoke the `collectTaxIds` method.
...
For more information on `incomplete` and `past_due` states, please refer to [our additional documentation on these states](<#our additional documentation on these states>).
...
To ensure that off-session payment confirmation notifications are delivered, verify that [Stripe webhooks are configured](<#Stripe webhooks are configured>) for your application and the `invoice.payment_action_required` webhook is enabled in your Stripe dashboard. In addition, your `Billable` model should also use Laravel's `Illuminate\Notifications\Notifiable` trait.
