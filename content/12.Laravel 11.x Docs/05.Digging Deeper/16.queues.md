---
modified: 2024-09-15T02:24:11-04:00
tags:
  - laravel
  - laravel11
  - docs
...
- [Introduction](#Introduction)
  ...
  - [Driver Notes and Prerequisites](<#Driver Notes and Prerequisites>)
...
- [Dispatching Jobs](<#Dispatching Jobs>)
  ...
  - [Jobs & Database Transactions](<#Jobs & Database Transactions>)
...
> [!NOTE]
> Laravel now offers Horizon, a beautiful dashboard and configuration system for your Redis powered queues. Check out the full [Horizon documentation](/12.Laravel%2011.x%20Docs/10.Packages/09.horizon) for more information.
...
In order to use the `database` queue driver, you will need a database table to hold the jobs. Typically, this is included in Laravel's default `0001_01_01_000002_create_jobs_table.php` [database migration](/12.Laravel%2011.x%20Docs/07.Database/04.migrations); however, if your application does not contain this migration, you may use the `make:queue-table` Artisan command to create it:
...
> [!NOTE]
> Job stubs may be customized using [stub publishing](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/01.artisan#stub-customization).
...
In this example, note that we were able to pass an [Eloquent model](/12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/01.eloquent) directly into the queued job's constructor. Because of the `Queueable` trait that the job is using, Eloquent models and their loaded relationships will be gracefully serialized and unserialized when the job is processing.
...
The `handle` method is invoked when the job is processed by the queue. Note that we are able to type-hint dependencies on the `handle` method of the job. The Laravel [service container](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/02.container) automatically injects these dependencies.

If you would like to take total control over how the container injects dependencies into the `handle` method, you may use the container's `bindMethod` method. The `bindMethod` method accepts a callback which receives the job and the container. Within the callback, you are free to invoke the `handle` method however you wish. Typically, you should call this method from the `boot` method of your `App\Providers\AppServiceProvider` [service provider](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/03.providers):
...
> [!WARNING]
> Unique jobs require a cache driver that supports [locks](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/03.cache#atomic-locks). Currently, the `memcached`, `redis`, `dynamodb`, `database`, `file`, and `array` cache drivers support atomic locks. In addition, unique job constraints do not apply to jobs within batches.
...
> [!NOTE]
> If you only need to limit the concurrent processing of a job, use the [`WithoutOverlapping`](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/16.queues#preventing-job-overlaps) job middleware instead.

<a name="encrypted-jobs"></a>

### Encrypted Jobs

Laravel allows you to ensure the privacy and integrity of a job's data via [encryption](/12.Laravel%2011.x%20Docs/06.Security/04.). To get started, simply add the `ShouldBeEncrypted` interface to the job class. Once this interface has been added to the class, Laravel will automatically encrypt your job before pushing it onto a queue:
...
As you can see, like [route middleware](/12.Laravel%2011.x%20Docs/04.The%20Basics/02.middleware), job middleware receive the job being processed and a callback that should be invoked to continue processing the job.
...
Although we just demonstrated how to write your own rate limiting job middleware, Laravel actually includes a rate limiting middleware that you may utilize to rate limit jobs. Like [route rate limiters](/12.Laravel%2011.x%20Docs/04.The%20Basics/01.routing#defining-rate-limiters), job rate limiters are defined using the `RateLimiter` facade's `for` method.
...
> [!WARNING]
> The `WithoutOverlapping` middleware requires a cache driver that supports [locks](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/03.cache#atomic-locks). Currently, the `memcached`, `redis`, `dynamodb`, `database`, `file`, and `array` cache drivers support atomic locks.
...
If a job exceeds its maximum number of attempts, it will be considered a "failed" job. For more information on handling failed jobs, consult the [failed job documentation](<#failed job documentation>). If `--tries=0` is provided to the `queue:work` command, the job will be retried indefinitely.
...
> [!NOTE]
> You may also define a `tries` property or `retryUntil` method on your [queued event listeners](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/07.events#queued-event-listeners).
...
> [!WARNING]
> The `pcntl` PHP extension must be installed in order to specify job timeouts. In addition, a job's "timeout" value should always be less than its ["retry after"](<#"retry after">) value. Otherwise, the job may be re-attempted before it has actually finished executing or timed out.

<a name="failing-on-timeout"></a>

#### Failing on Timeout

If you would like to indicate that a job should be marked as [failed](#failed) on timeout, you may define the `$failOnTimeout` property on the job class:
...
If an exception is thrown while the job is being processed, the job will automatically be released back onto the queue so it may be attempted again. The job will continue to be released until it has been attempted the maximum number of times allowed by your application. The maximum number of attempts is defined by the `--tries` switch used on the `queue:work` Artisan command. Alternatively, the maximum number of attempts may be defined on the job class itself. More information on running the queue worker [can be found below](<#can be found below>).
...
> [!NOTE]
> For more information on failed jobs, check out the [documentation on dealing with job failures](<#documentation on dealing with job failures>).
...
To define a batchable job, you should [create a queueable job](<#create a queueable job>) as normal; however, you should add the `Illuminate\Bus\Batchable` trait to the job class. This trait provides access to a `batch` method which may be used to retrieve the current batch that the job is executing within:
...
The batch's ID, which may be accessed via the `$batch->id` property, may be used to [query the Laravel command bus](<#query the Laravel command bus>) for information about the batch after it has been dispatched.
...
You may define a set of [chained jobs](<#chained jobs>) within a batch by placing the chained jobs within an array. For example, we may execute two job chains in parallel and execute a callback when both job chains have finished processing:
...
Conversely, you may run batches of jobs within a [chain](#chain) by defining batches within the chain. For example, you could first run a batch of jobs to release multiple podcasts then a batch of jobs to send the release notifications:
...
Without pruning, the `job_batches` table can accumulate records very quickly. To mitigate this, you should [schedule](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/19.scheduling) the `queue:prune-batches` Artisan command to run daily:
...
In addition, you may define `ttl` attribute for your table if you would like to take advantage of [automatic batch pruning](<#automatic batch pruning>).
...
Using the `catch` method, you may provide a closure that should be executed if the queued closure fails to complete successfully after exhausting all of your queue's [configured retry attempts](<#configured retry attempts>):
...
> [!NOTE]
> To keep the `queue:work` process running permanently in the background, you should use a process monitor such as [Supervisor](#Supervisor) to ensure that the queue worker does not stop running.
...
Remember, queue workers are long-lived processes and store the booted application state in memory. As a result, they will not notice changes in your code base after they have been started. So, during your deployment process, be sure to [restart your queue workers](<#restart your queue workers>). In addition, remember that any static state created or modified by your application will not be automatically reset between jobs.
...
The `--max-jobs` option may be used to instruct the worker to process the given number of jobs and then exit. This option may be useful when combined with [Supervisor](#Supervisor) so that your workers are automatically restarted after processing a given number of jobs, releasing any memory they may have accumulated:
...
The `--max-time` option may be used to instruct the worker to process jobs for the given number of seconds and then exit. This option may be useful when combined with [Supervisor](#Supervisor) so that your workers are automatically restarted after processing jobs for a given amount of time, releasing any memory they may have accumulated:
...
While your application is in [maintenance mode](/12.Laravel%2011.x%20Docs/02.Getting%20Started/02.configuration#maintenance-mode), no queued jobs will be handled. The jobs will continue to be handled as normal once the application is out of maintenance mode.
...
This command will instruct all queue workers to gracefully exit after they finish processing their current job so that no existing jobs are lost. Since the queue workers will exit when the `queue:restart` command is executed, you should be running a process manager such as [Supervisor](#Supervisor) to automatically restart the queue workers.

> [!NOTE]
> The queue uses the [cache](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/03.cache) to store restart signals, so you should verify that a cache driver is properly configured for your application before using this feature.
...
Sometimes your queued jobs will fail. Don't worry, things don't always go as planned! Laravel includes a convenient way to [specify the maximum number of times a job should be attempted](<#specify the maximum number of times a job should be attempted>). After an asynchronous job has exceeded this number of attempts, it will be inserted into the `failed_jobs` database table. [Synchronously dispatched jobs](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/16.queues#synchronous-dispatching) that fail are not stored in this table and their exceptions are immediately handled by the application.
...
> [!NOTE]
> When using [Horizon](/12.Laravel%2011.x%20Docs/10.Packages/09.horizon), you should use the `horizon:forget` command to delete a failed job instead of the `queue:forget` command.
...
> [!NOTE]
> When using [Horizon](/12.Laravel%2011.x%20Docs/10.Packages/09.horizon), you should use the `horizon:clear` command to clear jobs from the queue instead of the `queue:clear` command.
...
To get started, you should schedule the `queue:monitor` command to [run every minute](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/19.scheduling). The command accepts the names of the queues you wish to monitor as well as your desired job count threshold:
...
To test job chains, you will need to utilize the `Bus` facade's faking capabilities. The `Bus` facade's `assertChained` method may be used to assert that a [chain of jobs](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/16.queues#job-chaining) was dispatched. The `assertChained` method accepts an array of chained jobs as its first argument:
...
If a chained job [prepends or appends jobs to an existing chain](<#prepends or appends jobs to an existing chain>), you may use the job's `assertHasChain` method to assert that the job has the expected chain of remaining jobs:
...
If your job chain [contains a batch of jobs](<#contains a batch of jobs>), you may assert that the chained batch matches your expectations by inserting a `Bus::chainedBatch` definition within your chain assertion:
...
The `Bus` facade's `assertBatched` method may be used to assert that a [batch of jobs](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/16.queues#job-batching) was dispatched. The closure given to the `assertBatched` method receives an instance of `Illuminate\Bus\PendingBatch`, which may be used to inspect the jobs within the batch:
...
Sometimes, you may need to test that a queued job [releases itself back onto the queue](<#releases itself back onto the queue>). Or, you may need to test that the job deleted itself. You may test these queue interactions by instantiating the job and invoking the `withFakeQueueInteractions` method.
...
Using the `before` and `after` methods on the `Queue` [facade](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/05.facades), you may specify callbacks to be executed before or after a queued job is processed. These callbacks are a great opportunity to perform additional logging or increment statistics for a dashboard. Typically, you should call these methods from the `boot` method of a [service provider](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/03.providers). For example, we may use the `AppServiceProvider` that is included with Laravel:
...
Using the `looping` method on the `Queue` [facade](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/05.facades), you may specify callbacks that execute before the worker attempts to fetch a job from a queue. For example, you might register a closure to rollback any transactions that were left open by a previously failed job:
