---
modified: 2024-09-06T05:34:07-04:00
tags:
  - laravel
  - laravel11
  - docs
...
In addition to scheduling closures, you may also schedule [Artisan commands](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/01.artisan) and system commands. For example, you may use the `command` method to schedule an Artisan command using either the command's name or class.
...
The `job` method may be used to schedule a [queued job](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/16.queues). This method provides a convenient way to schedule queued jobs without using the `call` method to define closures to queue the job:
...
The `environments` method may be used to execute tasks only on the given environments (as defined by the `APP_ENV` [environment variable](/12.Laravel%2011.x%20Docs/02.Getting%20Started/02.configuration#environment-configuration)):
...
In this example, the `emails:send` [Artisan command](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/01.artisan) will be run every minute if it is not already running. The `withoutOverlapping` method is especially useful if you have tasks that vary drastically in their execution time, preventing you from predicting exactly how long a given task will take.

If needed, you may specify how many minutes must pass before the "without overlapping" lock expires. By default, the lock will expire after 24 hours:

    Schedule::command('emails:send')->withoutOverlapping(10);

Behind the scenes, the `withoutOverlapping` method utilizes your application's [cache](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/03.cache) to obtain locks. If necessary, you can clear these cache locks using the `schedule:clear-cache` Artisan command. This is typically only necessary if a task becomes stuck due to an unexpected server problem.
...
Using the `emailOutputTo` method, you may email the output to an email address of your choice. Before emailing the output of a task, you should configure Laravel's [email services](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/12.mail):
...
Laravel dispatches a variety of [events](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/07.events) during the scheduling process. You may [define listeners](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/07.events) for any of the following events:
