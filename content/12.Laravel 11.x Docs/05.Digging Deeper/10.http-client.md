---
modified: 2024-09-06T05:25:50-04:00
tags:
  - laravel
  - laravel11
  - docs
...
The `RequestSending` and `ConnectionFailed` events both contain a public `$request` property that you may use to inspect the `Illuminate\Http\Client\Request` instance. Likewise, the `ResponseReceived` event contains a `$request` property as well as a `$response` property which may be used to inspect the `Illuminate\Http\Client\Response` instance. You may create [event listeners](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/07.events) for these events within your application:
