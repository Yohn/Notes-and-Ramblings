---
modified: 2024-09-06T05:26:34-04:00
tags:
  - laravel
  - laravel11
  - docs
...
Notifications may be sent in two ways: using the `notify` method of the `Notifiable` trait or using the `Notification` [facade](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/05.facades). The `Notifiable` trait is included on your application's `App\Models\User` model by default:
...
Alternatively, you may send notifications via the `Notification` [facade](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/05.facades). This approach is useful when you need to send a notification to multiple notifiable entities such as a collection of users. To send notifications using the facade, pass all of the notifiable entities and the notification instance to the `send` method:
...
> [!WARNING]
> Before queueing notifications you should configure your queue and [start a worker](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/16.queues#running-the-queue-worker).
...
> [!NOTE]
> The `attach` method offered by notification mail messages also accepts [attachable objects](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/12.mail#attachable-objects). Please consult the comprehensive [attachable object documentation](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/12.mail#attachable-objects) to learn more.
...
Unlike attaching files in mailable objects, you may not attach a file directly from a storage disk using `attachFromStorage`. You should rather use the `attach` method with an absolute path to the file on the storage disk. Alternatively, you could return a [mailable](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/12.mail#generating-mailables) from the `toMail` method:
...
If needed, you may return a full [mailable object](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/12.mail) from your notification's `toMail` method. When returning a `Mailable` instead of a `MailMessage`, you will need to specify the message recipient using the mailable object's `to` method:
...
You can query the table to display the notifications in your application's user interface. But, before you can do that, you will need to create a database table to hold your notifications. You may use the `make:notifications-table` command to generate a [migration](/12.Laravel%2011.x%20Docs/07.Database/04.migrations) with the proper table schema:
...
> [!NOTE]
> If your notifiable models are using [UUID or ULID primary keys](/12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/01.eloquent#uuid-and-ulid-keys), you should replace the `morphs` method with [`uuidMorphs`](/12.Laravel%2011.x%20Docs/07.Database/04.migrations#column-method-uuidMorphs) or [`ulidMorphs`](/12.Laravel%2011.x%20Docs/07.Database/04.migrations#column-method-ulidMorphs) in the notification table migration.
...
Once notifications are stored in the database, you need a convenient way to access them from your notifiable entities. The `Illuminate\Notifications\Notifiable` trait, which is included on Laravel's default `App\Models\User` model, includes a `notifications` [Eloquent relationship](/12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/02.eloquent-relationships) that returns the notifications for the entity. To fetch notifications, you may access this method like any other Eloquent relationship. By default, notifications will be sorted by the `created_at` timestamp with the most recent notifications at the beginning of the collection:
...
Before broadcasting notifications, you should configure and be familiar with Laravel's [event broadcasting](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/02.broadcasting) services. Event broadcasting provides a way to react to server-side Laravel events from your JavaScript powered frontend.

<a name="formatting-broadcast-notifications"></a>

### Formatting Broadcast Notifications

The `broadcast` channel broadcasts notifications using Laravel's [event broadcasting](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/02.broadcasting) services, allowing your JavaScript powered frontend to catch notifications in realtime. If a notification supports broadcasting, you can define a `toBroadcast` method on the notification class. This method will receive a `$notifiable` entity and should return a `BroadcastMessage` instance. If the `toBroadcast` method does not exist, the `toArray` method will be used to gather the data that should be broadcast. The returned data will be encoded as JSON and broadcast to your JavaScript powered frontend. Let's take a look at an example `toBroadcast` method:
...
Notifications will broadcast on a private channel formatted using a `{notifiable}.{id}` convention. So, if you are sending a notification to an `App\Models\User` instance with an ID of `1`, the notification will be broadcast on the `App.Models.User.1` private channel. When using [Laravel Echo](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/02.broadcasting#client-side-installation), you may easily listen for notifications on a channel using the `notification` method:
...
If your application will be sending notifications to external Slack workspaces that are owned by your application's users, you will need to "distribute" your App via Slack. App distribution can be managed from your App's "Manage Distribution" tab within Slack. Once your App has been distributed, you may use [Socialite](/12.Laravel%2011.x%20Docs/10.Packages/22.socialite) to [obtain Slack Bot tokens](/12.Laravel%2011.x%20Docs/10.Packages/22.socialite#slack-bot-scopes) on behalf of your application's users.
...
> [!NOTE]
> Before sending notifications to external Slack workspaces, your Slack App must be [distributed](#distributed).

Of course, you will often want to send notifications to the Slack workspaces owned by your application's users. To do so, you will first need to obtain a Slack OAuth token for the user. Thankfully, [Laravel Socialite](/12.Laravel%2011.x%20Docs/10.Packages/22.socialite) includes a Slack driver that will allow you to easily authenticate your application's users with Slack and [obtain a bot token](/12.Laravel%2011.x%20Docs/10.Packages/22.socialite#slack-bot-scopes).
...
When a notification is sent, the `Illuminate\Notifications\Events\NotificationSent` [event](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/07.events) is dispatched by the notification system. This contains the "notifiable" entity and the notification instance itself. You may create [event listeners](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/07.events) for this event within your application:
