---
modified: 2024-09-06T05:38:27-04:00
tags:
  - laravel
  - laravel11
  - docs
...
Almost every modern web application interacts with a database. Laravel makes interacting with databases extremely simple across a variety of supported databases using raw SQL, a [fluent query builder](/12.Laravel%2011.x%20Docs/07.Database/02.queries), and the [Eloquent ORM](/12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/01.eloquent). Currently, Laravel provides first-party support for five databases:
...
By default, Laravel's sample [environment configuration](/12.Laravel%2011.x%20Docs/02.Getting%20Started/02.configuration#environment-configuration) is ready to use with [Laravel Sail](/12.Laravel%2011.x%20Docs/10.Packages/20.sail), which is a Docker configuration for developing Laravel applications on your local machine. However, you are free to modify your database configuration as needed for your local database.
...
> [!NOTE]
> If you use the [Laravel installer](/12.Laravel%2011.x%20Docs/02.Getting%20Started/01.installation#creating-a-laravel-project) to create your Laravel application and select SQLite as your database, Laravel will automatically create a `database/database.sqlite` file and run the default [database migrations](/12.Laravel%2011.x%20Docs/07.Database/04.migrations) for you.
...
A common performance bottleneck of modern web applications is the amount of time they spend querying databases. Thankfully, Laravel can invoke a closure or callback of your choice when it spends too much time querying the database during a single request. To get started, provide a query time threshold (in milliseconds) and closure to the `whenQueryingForLongerThan` method. You may invoke this method in the `boot` method of a [service provider](/12.Laravel%2011.x%20Docs/03.Architecture%20Concepts/03.providers):
...
> [!NOTE]
> The `DB` facade's transaction methods control the transactions for both the [query builder](/12.Laravel%2011.x%20Docs/07.Database/02.queries) and [Eloquent ORM](/12.Laravel%2011.x%20Docs/08.Eloquent%20ORM/01.eloquent).
...
To get started, you should schedule the `db:monitor` command to [run every minute](/12.Laravel%2011.x%20Docs/05.Digging%20Deeper/19.scheduling). The command accepts the names of the database connection configurations that you wish to monitor as well as the maximum number of open connections that should be tolerated before dispatching an event:
